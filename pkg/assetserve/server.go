...
func (m *httpServer) serveTLS(addr string, handler http.Handler) {
...
func (m *httpServer) serve(addr string, handler http.Handler) {
	server := &http.Server{Addr: addr, Handler: handler}
	go func() {
...
func (m *httpServer) graceShutdown(server *http.Server) {
...
func (m *httpServer) StartHttpServer() {
	if m.LocalAssets != "" {
		m.LocalAssets = strings.Replace(m.LocalAssets, "\\", "/", -1)
